...
for (i in seq(1000))
{
  ...
  edata4 <- qda(as.matrix(eddat[,c(2:11)]), eddat[,"Grp"], data = eddat, prior = c(1/3,1/3,1/3),
                subset = -inTrain, CV = T)
  Accuracies[i] <- confusionMatrix(eddat[-inTrain,"Grp"], edata4$class)$overall["Accuracy"]
}
summary(Accuracies)